...
void ABattleRoyaleProjectile::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	if (HasAuthority())
	{
		if (ABattleRoyaleCharacter* HitPlayer = Cast<ABattleRoyaleCharacter>(OtherActor))
		{
			if (ABattleRoyaleGameMode* GameMode = Cast<ABattleRoyaleGameMode>(GetWorld()->GetAuthGameMode()))
			{
				ABattleRoyaleCharacter* Killer = Cast<ABattleRoyaleCharacter>(GetOwner());
				GameMode->PlayerDied(HitPlayer, Killer);
